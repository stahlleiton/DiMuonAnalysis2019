  ...
  for (const auto& col : info.StrS.at("fitSystem")) {
	...
	if (!contain(info.Par, "N_"+objLabel) || info.Par.at("N_"+objLabel)=="") {
	  if (!contain(info.Par, "N_"+objFoundLabel) || info.Par.at("N_"+objFoundLabel)=="") {
        ...
        if (contain(info.Flag, "incMCTemp_"+mainObj+"_"+obj) && info.Flag.at("incMCTemp_"+mainObj+"_"+obj)) continue;
        //
        // CUTS FOR CUT AND COUNT ALGO
        if (!contain(info.Par, "Cut_"+objLabel) || info.Par.at("Cut_"+objLabel)=="") {
          if (!contain(info.Par, "Cut_"+objFoundLabel) || info.Par.at("Cut_"+objFoundLabel)=="") {
